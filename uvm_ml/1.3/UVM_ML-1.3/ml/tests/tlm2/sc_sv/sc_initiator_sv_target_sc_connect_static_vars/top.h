...
public :
  ...
  bool check_data(int base, PAYLOAD_TYPE* gp);
...
public :
  ...
  top(sc_module_name name_) : sc_module(name_),i("initiator") {
    ...
    cout << "[SC " << sc_time_stamp()<<"] About to connect sockets" << endl;
    uvm_ml_connect(full_initiator_socket_name, "uvm_test_top.env.consumer.target_socket");
  ...
  {
      cout << "[SC " << sc_time_stamp() << "] top: end of simulation" << endl;
  }

  void start_of_simulation()
  {
      cout << "[SC " << sc_time_stamp() <<  "] top: start of simulation" << endl;
  }

  void end_of_elaboration() 
  { 
      cout << "[SC " << sc_time_stamp() << "] top: end of elaboration" << endl;
  }

  void before_end_of_elaboration()
  { 
      cout << "[SC " << sc_time_stamp() << "] top: before end of elaboration" << endl;
  }

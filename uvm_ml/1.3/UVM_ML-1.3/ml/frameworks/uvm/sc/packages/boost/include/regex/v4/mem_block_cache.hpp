...
#ifndef UVMSC_BOOST_REGEX_V4_MEM_BLOCK_CACHE_HPP
#define UVMSC_BOOST_REGEX_V4_MEM_BLOCK_CACHE_HPP
...
struct mem_block_cache
{
   ...
   {
#ifdef UVMSC_BOOST_HAS_THREADS
      uvmsc_boost::static_mutex::scoped_lock g(mut);
#endif
     if(next)
      {
   ...
   void put(void* p)
   {
      ...
      uvmsc_boost::static_mutex::scoped_lock g(mut);
#endif
      if(cached_blocks >= UVMSC_BOOST_REGEX_MAX_CACHE_BLOCKS)
      {
         ::operator delete(p);
      }
      else

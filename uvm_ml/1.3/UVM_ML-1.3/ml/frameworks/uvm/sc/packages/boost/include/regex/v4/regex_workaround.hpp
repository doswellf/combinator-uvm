...
#define UVMSC_BOOST_REGEX_WORKAROUND_HPP
...
#include <packages/boost/include/scoped_ptr.hpp>
#include <packages/boost/include/scoped_array.hpp>
...
#ifdef UVMSC_BOOST_NO_STD_DISTANCE
template <class T>
...
#ifdef UVMSC_BOOST_REGEX_NO_BOOL
#  define UVMSC_BOOST_REGEX_MAKE_BOOL(x) static_cast<bool>((x) ? true : false)
#else
#  define UVMSC_BOOST_REGEX_MAKE_BOOL(x) static_cast<bool>(x)
#endif
...
#ifdef __cplusplus
namespace uvmsc_boost{ namespace re_detail{
#if UVMSC_BOOST_WORKAROUND(UVMSC_BOOST_MSVC,>=1400) && UVMSC_BOOST_WORKAROUND(UVMSC_BOOST_MSVC, <1600) && defined(_CPPLIB_VER) && defined(UVMSC_BOOST_DINKUMWARE_STDLIB) && !(defined(__SGI_STL_PORT) || defined(_STLPORT_VERSION))
   //
...
#elif UVMSC_BOOST_WORKAROUND(UVMSC_BOOST_MSVC, > 1500)
   //
   ...
   using std::equal; 
#endif 
#if UVMSC_BOOST_WORKAROUND(UVMSC_BOOST_MSVC,>=1400) && defined(__STDC_WANT_SECURE_LIB__) && __STDC_WANT_SECURE_LIB__ 

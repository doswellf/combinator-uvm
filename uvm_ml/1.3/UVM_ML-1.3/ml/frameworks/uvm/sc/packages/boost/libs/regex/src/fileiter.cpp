 ...
 */
  ...
  *   DESCRIPTION: Implements file io primitives + directory searching for class uvmsc_boost::RegEx.
  */
...
#include <packages/boost/include/regex/v4/fileiter.hpp>
...
#if (defined(__BORLANDC__) || defined(UVMSC_BOOST_REGEX_FI_WIN32_DIR) || defined(UVMSC_BOOST_MSVC)) && !defined(UVMSC_BOOST_RE_NO_WIN32)

// platform is DOS or Windows
// directories are separated with '\\'
// and names are insensitive of case

UVMSC_BOOST_REGEX_DECL const char* _fi_sep = "\\";
const char* _fi_sep_alt = "/";
#define UVMSC_BOOST_REGEX_FI_TRANSLATE(c) std::tolower(c)
...
#define UVMSC_BOOST_REGEX_FI_TRANSLATE(c) c

#endif

#ifdef UVMSC_BOOST_REGEX_FI_WIN32_MAP

void mapfile::open(const char* file)
{
#if defined(UVMSC_BOOST_NO_ANSI_APIS)
...
#else
      UVMSC_BOOST_REGEX_NOEH_ASSERT(hfile != INVALID_HANDLE_VALUE);
#endif
   }
}
...
#elif !defined(UVMSC_BOOST_RE_NO_STL)
...
{
   UVMSC_BOOST_ASSERT(node >= _first);
   UVMSC_BOOST_ASSERT(node <= _last);
   if(node < _last)
   {
      if(*node == 0)
      {
...
#else 
        UVMSC_BOOST_REGEX_NOEH_ASSERT((0 == std::ferror(hfile)) && (read_size != 0)); 
#endif 
      }
      else
      {
...
file_iterator::file_iterator()
{
   ...
   UVMSC_BOOST_REGEX_NOEH_ASSERT(_root)
   _path = new char[MAX_PATH];
   UVMSC_BOOST_REGEX_NOEH_ASSERT(_path)
   ptr = _path;
   *_path = 0;
   *_root = 0;
   ref = new file_iterator_ref();
   UVMSC_BOOST_REGEX_NOEH_ASSERT(ref)
...
#ifdef UVMSC_BOOST_REGEX_FI_POSIX_DIR
...
bool iswild(const char* mask, const char* name)
{
   while(*mask && *name)
   {
      ...
      case '.':
         ...
         // fall through:
      default:
         if(UVMSC_BOOST_REGEX_FI_TRANSLATE(*mask) != UVMSC_BOOST_REGEX_FI_TRANSLATE(*name))
            return false;
         ++mask;
         ++name;
         continue;

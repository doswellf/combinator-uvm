...
#ifndef UVMSC_BOOST_REGEX_V4_PERL_MATCHER_COMMON_HPP
#define UVMSC_BOOST_REGEX_V4_PERL_MATCHER_COMMON_HPP
...
{ 
   ...
   if(e.empty())
   {
      ...
      uvmsc_boost::throw_exception(ex);
   }
   pstate = 0;
   m_match_flags = f;
...
void perl_matcher<BidiIterator, Allocator, traits>::estimate_max_state_count(std::random_access_iterator_tag*)
{
   ...
   std::ptrdiff_t dist = uvmsc_boost::re_detail::distance(base, last);
   if(dist == 0)
      dist = 1;
   ...
   {
      max_state_count = (std::min)((std::ptrdiff_t)UVMSC_BOOST_REGEX_MAX_STATE_COUNT, (std::numeric_limits<std::ptrdiff_t>::max)() - 2);
      return;
   }
   states *= dist;
   ...
   //
   if(states > UVMSC_BOOST_REGEX_MAX_STATE_COUNT)
      states = UVMSC_BOOST_REGEX_MAX_STATE_COUNT;
   //
...
inline void perl_matcher<BidiIterator, Allocator, traits>::estimate_max_state_count(void*)
{
   ...
   max_state_count = UVMSC_BOOST_REGEX_MAX_STATE_COUNT;
}
...
{
   ::uvmsc_boost::re_detail::concrete_protected_call
...
bool perl_matcher<BidiIterator, Allocator, traits>::match_imp()
{
   ...
   used_block_count = UVMSC_BOOST_REGEX_MAX_BLOCKS;
#if !defined(UVMSC_BOOST_NO_EXCEPTIONS)
   try{
#endif
#endif
...
#if defined(UVMSC_BOOST_REGEX_NON_RECURSIVE) && !defined(UVMSC_BOOST_NO_EXCEPTIONS)
   }
   catch(...)
   {
...
bool perl_matcher<BidiIterator, Allocator, traits>::match_word_boundary()
{
   ...
   #if defined(UVMSC_BOOST_REGEX_USE_C_LOCALE) && defined(__GNUC__) && (__GNUC__ == 2) && (__GNUC_MINOR__ < 95)
...
bool perl_matcher<BidiIterator, Allocator, traits>::match_backref()
{
   ...
   if(index >= 10000)
   {
      ...
      UVMSC_BOOST_ASSERT(r.first != r.second);
      do
      {
         index = r.first->index;
         ++r.first;
...
#pragma warning(disable:4127)
   ...
   {
      std::ptrdiff_t maxlen = ::uvmsc_boost::re_detail::distance(backstop, position);

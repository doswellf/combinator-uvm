 ...
 */
  ...
  *   DESCRIPTION: Implements high level class uvmsc_boost::RexEx
  */
...
#if !defined(UVMSC_BOOST_NO_STD_STRING)
#include <map>
#include <list>
#include <packages/boost/include/regex/v4/fileiter.hpp>
typedef uvmsc_boost::match_flag_type match_flag_type;
#include <cstdio>
...
{
   uvmsc_boost::uint_fast32_t f = icase ? regex::normal | regex::icase : regex::normal;
...
namespace std{
template<> template<>
basic_string<char>& UVMSC_BOOST_REGEX_DECL
...
template<> template<>
basic_string<wchar_t>& UVMSC_BOOST_REGEX_DECL

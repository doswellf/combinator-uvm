...
#define UVMSC_BOOST_REGEX_V4_MATCH_RESULTS_HPP
...
public: 
...
#if  !defined(UVMSC_BOOST_NO_STD_ALLOCATOR) && !(defined(UVMSC_BOOST_MSVC) && defined(_STLPORT_VERSION))
   ...
   difference_type position(size_type sub = 0) const
   {
      ...
      if(sub < m_subs.size())
      {
         ...
         if(s.matched || (sub == 2))
         {
            return ::uvmsc_boost::re_detail::distance((BidiIterator)(m_base), (BidiIterator)(s.first));
         }
      }
   ...
   {
      UVMSC_BOOST_STATIC_ASSERT(sizeof(charT) <= sizeof(char_type));
      if(i == j)
         return m_null;
                         ...
                         const RegexT& re) const
   {
      if(m_is_singular)
         raise_logic_error();
      typedef ::uvmsc_boost::regex_traits_wrapper<typename RegexT::traits_type> traits_type;
   ...
   void UVMSC_BOOST_REGEX_CALL set_second(BidiIterator i)
   {
      UVMSC_BOOST_ASSERT(m_subs.size() > 2);
      m_subs[2].second = i;
   ...
   void UVMSC_BOOST_REGEX_CALL set_second(BidiIterator i, size_type pos, bool m = true, bool escape_k = false)
   {
      if(pos)
         m_last_closed_paren = static_cast<int>(pos);
      pos += 2;
      UVMSC_BOOST_ASSERT(m_subs.size() > pos);
      m_subs[pos].second = i;
   ...
   void UVMSC_BOOST_REGEX_CALL set_size(size_type n, BidiIterator i, BidiIterator j)
   {
      value_type v(j);
   ...
   void UVMSC_BOOST_REGEX_CALL set_base(BidiIterator pos)
   {
      m_base = pos;
   }
   ...
   void UVMSC_BOOST_REGEX_CALL set_first(BidiIterator i)
   {
      UVMSC_BOOST_ASSERT(m_subs.size() > 2);
      // set up prefix:
      m_subs[1].second = i;
   ...
   void UVMSC_BOOST_REGEX_CALL set_first(BidiIterator i, size_type pos, bool escape_k = false)
   {
      UVMSC_BOOST_ASSERT(pos+2 < m_subs.size());
      if(pos || escape_k)
      {
   ...
   void UVMSC_BOOST_REGEX_CALL maybe_assign(const match_results<BidiIterator, Allocator>& m);

   void UVMSC_BOOST_REGEX_CALL set_named_subs(uvmsc_boost::shared_ptr<named_sub_type> subs)
   {
      m_named_subs = subs;
   }
...
private:
   ...
   {
      std::logic_error e("Attempt to access an uninitialzed uvmsc_boost::match_results<> class.");
   ...
   uvmsc_boost::shared_ptr<named_sub_type> m_named_subs;     // Shared copy of named subs in the regex object
   int m_last_closed_paren;                            // Last ) to be seen - used for formatting
   bool m_is_singular;                                 // True if our stored iterators are singular
};

template <class BidiIterator, class Allocator>
void UVMSC_BOOST_REGEX_CALL match_results<BidiIterator, Allocator>::maybe_assign(const match_results<BidiIterator, Allocator>& m)
{
   if(m_is_singular)
   {
      *this = m;
   ...
   for(i = 0; i < size(); ++i, ++p1, ++p2)
   {
      ...
      base1 = ::uvmsc_boost::re_detail::distance(l_base, p1->first);
      base2 = ::uvmsc_boost::re_detail::distance(l_base, p2->first);
      UVMSC_BOOST_ASSERT(base1 >= 0);
      UVMSC_BOOST_ASSERT(base2 >= 0);
      if(base1 < base2) return;
      if(base2 < base1) break;

      len1 = ::uvmsc_boost::re_detail::distance((BidiIterator)p1->first, (BidiIterator)p1->second);
      len2 = ::uvmsc_boost::re_detail::distance((BidiIterator)p2->first, (BidiIterator)p2->second);
      UVMSC_BOOST_ASSERT(len1 >= 0);
      UVMSC_BOOST_ASSERT(len2 >= 0);

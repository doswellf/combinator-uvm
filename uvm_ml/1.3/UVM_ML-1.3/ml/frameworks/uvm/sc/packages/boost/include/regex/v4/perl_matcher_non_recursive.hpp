...
#ifndef UVMSC_BOOST_REGEX_V4_PERL_MATCHER_NON_RECURSIVE_HPP
#define UVMSC_BOOST_REGEX_V4_PERL_MATCHER_NON_RECURSIVE_HPP
...
struct save_state_init
{
      ...
      : stack(base)
   {
      ...
      *end = reinterpret_cast<saved_state*>(reinterpret_cast<char*>(*base)+UVMSC_BOOST_REGEX_BLOCKSIZE);
      --(*end);
      (void) new (*end)saved_state(0);
      UVMSC_BOOST_ASSERT(*end > *base);
   }
   ~save_state_init()
   {
      put_mem_block(*stack);
...
bool perl_matcher<BidiIterator, Allocator, traits>::match_all_states()
{
   static matcher_proc_type const s_match_vtable[30] = 
   {
      ...
      //(::uvmsc_boost::is_random_access_iterator<BidiIterator>::value ? &perl_matcher<BidiIterator, Allocator, traits>::match_dot_repeat_fast : &perl_matcher<BidiIterator, Allocator, traits>::match_dot_repeat_slow),
...
void perl_matcher<BidiIterator, Allocator, traits>::extend_stack()
{
   if(used_block_count)
   {
      ...
      backup_state = reinterpret_cast<saved_state*>(reinterpret_cast<char*>(stack_base)+UVMSC_BOOST_REGEX_BLOCKSIZE);
...
bool perl_matcher<BidiIterator, Allocator, traits>::match_startmark()
{
...
#ifdef UVMSC_BOOST_REGEX_MATCH_EXTRA
   ...
   case -4:
      ...
      UVMSC_BOOST_ASSERT(alt->type == syntax_element_alt);
      pstate = alt->next.p;
      ...
      else
      {
         ...
         UVMSC_BOOST_ASSERT(pstate->type == syntax_element_startmark);
...
bool perl_matcher<BidiIterator, Allocator, traits>::match_dot_repeat_fast()
{
   ...
   const re_repeat* rep = static_cast<const re_repeat*>(pstate);
   bool greedy = (rep->greedy) && (!(m_match_flags & regex_constants::match_any) || m_independent);   
   unsigned count = static_cast<unsigned>((std::min)(static_cast<unsigned>(::uvmsc_boost::re_detail::distance(position, last)), static_cast<unsigned>(greedy ? rep->max : rep->min)));
   if(rep->min > count)
   {
      position = last;
...
#pragma option push -w-8008 -w-8066 -w-8004
#endif
   const re_repeat* rep = static_cast<const re_repeat*>(pstate);
   UVMSC_BOOST_ASSERT(1 == static_cast<const re_literal*>(rep->next.p)->length);
   ...
   if(::uvmsc_boost::is_random_access_iterator<BidiIterator>::value)
   {
      BidiIterator end = position;
      std::advance(end, (std::min)((std::size_t)::uvmsc_boost::re_detail::distance(position, last), desired));
      BidiIterator origin(position);
      while((position != end) && (traits_inst.translate(*position, icase) == what))
      {
         ++position;
      }
      count = (unsigned)::uvmsc_boost::re_detail::distance(origin, position);
   }
   else
   {
...
{
   UVMSC_BOOST_ASSERT(pstate->type == syntax_element_recurse);
   //
   // Backup call stack:
   //
   push_recursion_pop();
...
bool perl_matcher<BidiIterator, Allocator, traits>::match_match()
{
   if(!recursion_stack.empty())
   {
      UVMSC_BOOST_ASSERT(0 == recursion_stack.back().idx);
      pstate = recursion_stack.back().preturn_address;
      *m_presult = recursion_stack.back().results;
      push_recursion(recursion_stack.back().idx, recursion_stack.back().preturn_address, &recursion_stack.back().results);
      recursion_stack.pop_back();
...
bool perl_matcher<BidiIterator, Allocator, traits>::unwind_paren(bool have_match)
{
   ...
   uvmsc_boost::re_detail::inplace_destroy(pmp);
...
{
   uvmsc_boost::re_detail::inplace_destroy(m_backup_state++);
   pstate = 0;   // nothing left to search
   return false; // end of stack nothing more to search
}
...
inline void perl_matcher<BidiIterator, Allocator, traits>::destroy_single_repeat()
{
   ...
   uvmsc_boost::re_detail::inplace_destroy(p++);
   m_backup_state = p;
}
...
bool perl_matcher<BidiIterator, Allocator, traits>::unwind_greedy_single_repeat(bool r)
{
   ...
   UVMSC_BOOST_ASSERT(rep->next.p != 0);
   UVMSC_BOOST_ASSERT(rep->alt.p != 0);
...
bool perl_matcher<BidiIterator, Allocator, traits>::unwind_slow_dot_repeat(bool r)
{
   ...
   const re_repeat* rep = pmp->rep;
   std::size_t count = pmp->count;
   UVMSC_BOOST_ASSERT(rep->type == syntax_element_dot_rep);
   UVMSC_BOOST_ASSERT(rep->next.p != 0);
   UVMSC_BOOST_ASSERT(rep->alt.p != 0);
   UVMSC_BOOST_ASSERT(rep->next.p->type == syntax_element_wild);

   UVMSC_BOOST_ASSERT(count < rep->max);
   pstate = rep->next.p;
   position = pmp->last_position;
...
bool perl_matcher<BidiIterator, Allocator, traits>::unwind_char_repeat(bool r)
{
   ...
   UVMSC_BOOST_ASSERT(rep->type == syntax_element_char_rep);
   UVMSC_BOOST_ASSERT(rep->next.p != 0);
   UVMSC_BOOST_ASSERT(rep->alt.p != 0);
   UVMSC_BOOST_ASSERT(rep->next.p->type == syntax_element_literal);
   UVMSC_BOOST_ASSERT(count < rep->max);
...
bool perl_matcher<BidiIterator, Allocator, traits>::unwind_short_set_repeat(bool r)
{
   ...
   UVMSC_BOOST_ASSERT(rep->type == syntax_element_short_set_rep);
   UVMSC_BOOST_ASSERT(rep->next.p != 0);
   UVMSC_BOOST_ASSERT(rep->alt.p != 0);
   UVMSC_BOOST_ASSERT(rep->next.p->type == syntax_element_set);
   UVMSC_BOOST_ASSERT(count < rep->max);
...
bool perl_matcher<BidiIterator, Allocator, traits>::unwind_long_set_repeat(bool r)
{
   ...
   UVMSC_BOOST_ASSERT(rep->type == syntax_element_long_set_rep);
   UVMSC_BOOST_ASSERT(rep->next.p != 0);
   UVMSC_BOOST_ASSERT(rep->alt.p != 0);
   UVMSC_BOOST_ASSERT(rep->next.p->type == syntax_element_long_set);
   UVMSC_BOOST_ASSERT(count < rep->max);

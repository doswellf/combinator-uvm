...
typedef uvmsc_boost::basic_regex<char, c_regex_traits<char> > c_regex_type;

UVMSC_BOOST_REGEX_DECL int UVMSC_BOOST_REGEX_CCALL regcompA(regex_tA* expression, const char* ptr, int f)
{
   ...
   // set default flags:
   uvmsc_boost::uint_fast32_t flags = (f & REG_PERLEX) ? 0 : ((f & REG_EXTENDED) ? regex::extended : regex::basic);
...
UVMSC_BOOST_REGEX_DECL regsize_t UVMSC_BOOST_REGEX_CCALL regerrorA(int code, const regex_tA* e, char* buf, regsize_t buf_size)
{
   std::size_t result = 0;
   if(code & REG_ITOA)
   {
   ...
   if(code <= (int)REG_E_UNKNOWN)
   {
      ...
      if((e) && (e->re_magic == magic_value))
         p = static_cast<c_regex_type*>(e->guts)->get_traits().error_string(static_cast< ::uvmsc_boost::regex_constants::error_type>(code));
      else
      {
...
UVMSC_BOOST_REGEX_DECL int UVMSC_BOOST_REGEX_CCALL regexecA(const regex_tA* expression, const char* buf, regsize_t n, regmatch_t* array, int eflags)
{
...
UVMSC_BOOST_REGEX_DECL void UVMSC_BOOST_REGEX_CCALL regfreeA(regex_tA* expression)
{
   if(expression->re_magic == magic_value)
   {

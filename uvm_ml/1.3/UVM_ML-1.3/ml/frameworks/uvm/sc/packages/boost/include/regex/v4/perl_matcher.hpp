...
#define UVMSC_BOOST_REGEX_MATCHER_HPP

#include <packages/boost/include/regex/v4/iterator_category.hpp>
...
// error checking API:
//
UVMSC_BOOST_REGEX_DECL void UVMSC_BOOST_REGEX_CALL verify_options(uvmsc_boost::regex_constants::syntax_option_type ef, match_flag_type mf);
//
// function can_start:
//
...
#if defined(WCHAR_MIN) && (WCHAR_MIN == 0) && !defined(UVMSC_BOOST_NO_INTRINSIC_WCHAR_T)
...
//
#ifndef _RWSTD_VER
#if !UVMSC_BOOST_WORKAROUND(UVMSC_BOOST_MSVC, < 1310)
...
iterator UVMSC_BOOST_REGEX_CALL re_is_set_member(iterator next, 
                          iterator last, 
...
{   
   ...
   const ::uvmsc_boost::regex_traits_wrapper<traits_type>& traits_inst = *(e.m_ptraits);
...
private:
   ...
   {
      return ::uvmsc_boost::is_random_access_iterator<BidiIterator>::value ? match_dot_repeat_fast() : match_dot_repeat_slow();
   }
   bool match_backstep();
...
private:
   ...
   const ::uvmsc_boost::regex_traits_wrapper<traits>& traits_inst;
...
//
#ifdef UVMSC_BOOST_REGEX_RECURSIVE
#include <packages/boost/include/regex/v4/perl_matcher_recursive.hpp>
#else
#include <packages/boost/include/regex/v4/perl_matcher_non_recursive.hpp>
#endif
// this one has to be last:
#include <packages/boost/include/regex/v4/perl_matcher_common.hpp>

#ifndef UVMSC_BOOST_TT_IS_ABSTRACT_CLASS_HPP
#define UVMSC_BOOST_TT_IS_ABSTRACT_CLASS_HPP
...
//              and John Maddock.
//  - Dec 2004: Added new config macro UVMSC_BOOST_NO_IS_ABSTRACT which causes the template
//              to degrade gracefully, rather than trash the compiler (John Maddock).
//

#include <packages/boost/include/type_traits/intrinsics.hpp>
#ifndef UVMSC_BOOST_IS_ABSTRACT
...
#ifdef UVMSC_BOOST_NO_IS_ABSTRACT
#include <packages/boost/include/type_traits/is_polymorphic.hpp>
#endif
#endif
...
#ifdef UVMSC_BOOST_IS_ABSTRACT
template <class T>
struct is_abstract_imp
{
   UVMSC_BOOST_STATIC_CONSTANT(bool, value = UVMSC_BOOST_IS_ABSTRACT(T));
};
#elif !defined(UVMSC_BOOST_NO_IS_ABSTRACT)
template<class T>
struct is_abstract_imp2
{
   ...
   //
   UVMSC_BOOST_STATIC_ASSERT(sizeof(T) != 0);

   // GCC2 won't even parse this template if we embed the computation
   // of s1 in the computation of value.
#ifdef __GNUC__
   UVMSC_BOOST_STATIC_CONSTANT(std::size_t, s1 = sizeof(is_abstract_imp2<T>::template check_sig<T>(0)));
#else
#if UVMSC_BOOST_WORKAROUND(UVMSC_BOOST_MSVC_FULL_VER, >= 140050000)
#pragma warning(push)
#pragma warning(disable:6334)
#endif
   UVMSC_BOOST_STATIC_CONSTANT(std::size_t, s1 = sizeof(check_sig<T>(0)));
#if UVMSC_BOOST_WORKAROUND(UVMSC_BOOST_MSVC_FULL_VER, >= 140050000)
#pragma warning(pop)
#endif
#endif
...
struct is_abstract_imp
{
   typedef is_abstract_select< ::uvmsc_boost::is_class<T>::value> selector;
   typedef typename selector::template rebind<T> binder;
   typedef typename binder::type type;

   UVMSC_BOOST_STATIC_CONSTANT(bool, value = type::value);
};
...
UVMSC_BOOST_TT_AUX_BOOL_TRAIT_DEF1(is_abstract,T,::uvmsc_boost::detail::is_abstract_imp<T>::value)
#else
UVMSC_BOOST_TT_AUX_BOOL_TRAIT_DEF1(is_abstract,T,::uvmsc_boost::detail::is_polymorphic_imp<T>::value)
#endif
...
#endif //UVMSC_BOOST_TT_IS_ABSTRACT_CLASS_HPP

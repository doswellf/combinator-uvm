...
#define UVMSC_BOOST_REGEX_V4_BASIC_REGEX_CREATOR_HPP
...
struct digraph : public std::pair<charT, charT>
{
   ...
   {
      UVMSC_BOOST_ASSERT(s.size() <= 2);
      UVMSC_BOOST_ASSERT(s.size());
      this->first = s[0];
...
protected:
   ...
   const ::uvmsc_boost::regex_traits_wrapper<traits>&  
   ...
   uvmsc_boost::uintmax_t              m_bad_repeats;        // bitmask of repeats we can't deduce a startmap for;
   ...
   : m_pdata(data), m_traits(*(data->m_ptraits)), m_last_state(0), m_repeater_id(0), m_has_backrefs(false), m_backrefs(0), m_has_recursions(false)
{
   ...
   m_pdata->m_status = ::uvmsc_boost::regex_constants::error_ok;
   ...
   UVMSC_BOOST_ASSERT(m_word_mask != 0); 
   UVMSC_BOOST_ASSERT(m_mask_space != 0); 
   UVMSC_BOOST_ASSERT(m_lower_mask != 0); 
   UVMSC_BOOST_ASSERT(m_upper_mask != 0); 
   UVMSC_BOOST_ASSERT(m_alpha_mask != 0); 
}
   ...
   const basic_char_set<charT, traits>& char_set, mpl::false_*)
{
   ...
   // fill in the basics:
   //
   result->csingles = static_cast<unsigned int>(::uvmsc_boost::re_detail::distance(char_set.singles_begin(), char_set.singles_end()));
   result->cranges = static_cast<unsigned int>(::uvmsc_boost::re_detail::distance(char_set.ranges_begin(), char_set.ranges_end())) / 2;
   result->cequivalents = static_cast<unsigned int>(::uvmsc_boost::re_detail::distance(char_set.equivalents_begin(), char_set.equivalents_end()));
   ...
   const basic_char_set<charT, traits>& char_set, mpl::true_*)
{
   ...
   while(first != last)
   {
      ...
      if(flags() & regex_constants::collate)
      {
         ...
         UVMSC_BOOST_ASSERT(c3[1] == charT(0));
   ...
   {
      string_type s;
      UVMSC_BOOST_ASSERT(static_cast<charT>(0) == first->second);
      s = m_traits.transform_primary(&first->first, &first->first+1);
      if(s.empty())
         return 0;  // invalid or unsupported equivalence class
      for(unsigned i = 0; i < (1u << CHAR_BIT); ++i)
               ...
               if(idx >= 10000)
               {
                  ...
                  if(idx <= 0)
                  {
                     ...
                     if(0 == this->m_pdata->m_status) // update the error code if not already set
                        this->m_pdata->m_status = uvmsc_boost::regex_constants::error_bad_pattern;
                     //
                     ...
                     if(0 == (this->flags() & regex_constants::no_except))
                     {
                        ...
                        uvmsc_boost::regex_error e(message, uvmsc_boost::regex_constants::error_bad_pattern, 0);
                        e.raise();
                     }
                  }
               }
   ...
   case syntax_element_long_set_rep:
      {
         ...
         static const uvmsc_boost::uintmax_t one = 1uL;

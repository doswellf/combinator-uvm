 ...
 */
  ...
  *   FILE         packages/boost/include/regex/config/borland.hpp
...
#        ifdef UVMSC_BOOST_REGEX_BUILD_DLL
...
#  endif
#  if (__BORLANDC__ <= 0x540) && !defined(UVMSC_BOOST_REGEX_NO_LIB) && !defined(_NO_VCL)
      // C++ Builder 4 and earlier, we can't tell whether we should be using
      // the VCL runtime or not, do a static link instead:
#     define UVMSC_BOOST_REGEX_STATIC_LINK
#  endif
   //
   // VCL support:
   // if we're building a console app then there can't be any VCL (can there?)
#  if !defined(__CONSOLE__) && !defined(_NO_VCL)
#     define UVMSC_BOOST_REGEX_USE_VCL
#  endif
   //
   ...
   //
#  ifndef _Windows
#     ifndef UVMSC_BOOST_REGEX_NO_LIB
#        define UVMSC_BOOST_REGEX_NO_LIB
#     endif
#     ifndef UVMSC_BOOST_REGEX_STATIC_LINK
#        define UVMSC_BOOST_REGEX_STATIC_LINK
#     endif
#  endif

...
#ifndef UVMSC_BOOST_REGEX_ICU_HPP
#define UVMSC_BOOST_REGEX_ICU_HPP

#include <unicode/utypes.h>
#include <unicode/uchar.h>
#include <unicode/coll.h>
#include <packages/boost/include/regex.hpp>
#include <packages/boost/include/regex/pending/unicode_iterator.hpp>
...
//
class UVMSC_BOOST_REGEX_DECL icu_regex_traits_implementation
{
...
private:
   ...
   uvmsc_boost::scoped_ptr< U_NAMESPACE_QUALIFIER Collator> m_collator;          // The full collation object
   uvmsc_boost::scoped_ptr< U_NAMESPACE_QUALIFIER Collator> m_primary_collator;  // The primary collation object
};

inline uvmsc_boost::shared_ptr<icu_regex_traits_implementation> get_icu_regex_traits_implementation(const U_NAMESPACE_QUALIFIER Locale& loc)
{
   return uvmsc_boost::shared_ptr<icu_regex_traits_implementation>(new icu_regex_traits_implementation(loc));
}

}

class UVMSC_BOOST_REGEX_DECL icu_regex_traits
{
public:
   typedef UChar32                      char_type;
   typedef std::size_t                  size_type;
   typedef std::vector<char_type>       string_type;
   typedef U_NAMESPACE_QUALIFIER Locale locale_type;
#ifdef UVMSC_BOOST_NO_INT64_T
   typedef std::bitset<64>              char_class_type;
#else
   typedef uvmsc_boost::uint64_t              char_class_type;
#endif
   ...
   std::string error_string(::uvmsc_boost::regex_constants::error_type n) const
   {
...
private:
   ...
   uvmsc_boost::shared_ptr< ::uvmsc_boost::re_detail::icu_regex_traits_implementation> m_pimpl;
};
...
// template instances:
//
#define UVMSC_BOOST_REGEX_CHAR_T UChar32
#undef UVMSC_BOOST_REGEX_TRAITS_T
#define UVMSC_BOOST_REGEX_TRAITS_T , icu_regex_traits
#define UVMSC_BOOST_REGEX_ICU_INSTANCES
#ifdef UVMSC_BOOST_REGEX_ICU_INSTANTIATE
#  define UVMSC_BOOST_REGEX_INSTANTIATE
#endif
#include <packages/boost/include/regex/v4/instances.hpp>
#undef UVMSC_BOOST_REGEX_CHAR_T
#undef UVMSC_BOOST_REGEX_TRAITS_T
#undef UVMSC_BOOST_REGEX_ICU_INSTANCES
...
#if !defined(UVMSC_BOOST_NO_MEMBER_TEMPLATES) && !defined(__IBMCPP__)
template <class InputIterator>
inline u32regex do_make_u32regex(InputIterator i, 
                              InputIterator j, 
                              uvmsc_boost::regex_constants::syntax_option_type opt, 
                              const uvmsc_boost::mpl::int_<1>*)
{
   typedef uvmsc_boost::u8_to_u32_iterator<InputIterator, UChar32> conv_type;
...
inline u32regex do_make_u32regex(InputIterator i, 
                              InputIterator j, 
                              uvmsc_boost::regex_constants::syntax_option_type opt, 
                              const uvmsc_boost::mpl::int_<2>*)
{
   typedef uvmsc_boost::u16_to_u32_iterator<InputIterator, UChar32> conv_type;
   return u32regex(conv_type(i, i, j), conv_type(j, i, j), opt);
}
...
inline u32regex make_u32regex(InputIterator i, 
                              ...
                              uvmsc_boost::regex_constants::syntax_option_type opt)
{
   return re_detail::do_make_u32regex(i, j, opt, static_cast<uvmsc_boost::mpl::int_<sizeof(*i)> const*>(0));
}
//
// construction from UTF-8 nul-terminated strings:
//
inline u32regex make_u32regex(const char* p, uvmsc_boost::regex_constants::syntax_option_type opt = uvmsc_boost::regex_constants::perl)
{
   return re_detail::do_make_u32regex(p, p + std::strlen(p), opt, static_cast<uvmsc_boost::mpl::int_<1> const*>(0));
}
inline u32regex make_u32regex(const unsigned char* p, uvmsc_boost::regex_constants::syntax_option_type opt = uvmsc_boost::regex_constants::perl)
{
   return re_detail::do_make_u32regex(p, p + std::strlen(reinterpret_cast<const char*>(p)), opt, static_cast<uvmsc_boost::mpl::int_<1> const*>(0));
}
//
// construction from UTF-16 nul-terminated strings:
//
#ifndef UVMSC_BOOST_NO_WREGEX
inline u32regex make_u32regex(const wchar_t* p, uvmsc_boost::regex_constants::syntax_option_type opt = uvmsc_boost::regex_constants::perl)
{
   return re_detail::do_make_u32regex(p, p + std::wcslen(p), opt, static_cast<uvmsc_boost::mpl::int_<sizeof(wchar_t)> const*>(0));
}
#endif
#if !defined(U_WCHAR_IS_UTF16) && (U_SIZEOF_WCHAR_T != 2)
inline u32regex make_u32regex(const UChar* p, uvmsc_boost::regex_constants::syntax_option_type opt = uvmsc_boost::regex_constants::perl)
{
   return re_detail::do_make_u32regex(p, p + u_strlen(p), opt, static_cast<uvmsc_boost::mpl::int_<2> const*>(0));
}
#endif
//
...
inline u32regex make_u32regex(const std::basic_string<C, T, A>& s, uvmsc_boost::regex_constants::syntax_option_type opt = uvmsc_boost::regex_constants::perl)
{
   return re_detail::do_make_u32regex(s.begin(), s.end(), opt, static_cast<uvmsc_boost::mpl::int_<sizeof(C)> const*>(0));
}
//
// Construction from ICU string type:
//
inline u32regex make_u32regex(const U_NAMESPACE_QUALIFIER UnicodeString& s, uvmsc_boost::regex_constants::syntax_option_type opt = uvmsc_boost::regex_constants::perl)
{
   return re_detail::do_make_u32regex(s.getBuffer(), s.getBuffer() + s.length(), opt, static_cast<uvmsc_boost::mpl::int_<2> const*>(0));
}
...
inline bool do_regex_match(BidiIterator first, BidiIterator last, 
                 ...
                 uvmsc_boost::mpl::int_<4> const*)
{
   return ::uvmsc_boost::regex_match(first, last, m, e, flags);
}
template <class BidiIterator, class Allocator>
bool do_regex_match(BidiIterator first, BidiIterator last, 
                 ...
                 const u32regex& e, 
                 match_flag_type flags,
                 uvmsc_boost::mpl::int_<2> const*)
{
   typedef u16_to_u32_iterator<BidiIterator, UChar32> conv_type;
   typedef match_results<conv_type>                   match_type;
   typedef typename match_type::allocator_type        alloc_type;
   match_type what;
   bool result = ::uvmsc_boost::regex_match(conv_type(first, first, last), conv_type(last, first, last), what, e, flags);
...
#if !defined(U_WCHAR_IS_UTF16) && (U_SIZEOF_WCHAR_T != 2) && !defined(UVMSC_BOOST_NO_WREGEX)
...
#ifndef UVMSC_BOOST_NO_STD_WSTRING
...
inline bool do_regex_search(BidiIterator first, BidiIterator last, 
                 ...
                 uvmsc_boost::mpl::int_<4> const*)
{
   return ::uvmsc_boost::regex_search(first, last, m, e, flags, base);
}
template <class BidiIterator, class Allocator>
                 ...
                 uvmsc_boost::mpl::int_<2> const*)
{
   ...
   typedef typename match_type::allocator_type        alloc_type;
   match_type what;
   bool result = ::uvmsc_boost::regex_search(conv_type(first, first, last), conv_type(last, first, last), what, e, flags, conv_type(base));
   // copy results across to m:
   if(result) copy_results(m, what);
   return result;
}
...
namespace re_detail{
template <class I>
inline std::pair< uvmsc_boost::u8_to_u32_iterator<I>, uvmsc_boost::u8_to_u32_iterator<I> >
   make_utf32_seq(I i, I j, mpl::int_<1> const*)
{
   return std::pair< uvmsc_boost::u8_to_u32_iterator<I>, uvmsc_boost::u8_to_u32_iterator<I> >(uvmsc_boost::u8_to_u32_iterator<I>(i, i, j), uvmsc_boost::u8_to_u32_iterator<I>(j, i, j));
}
template <class I>
inline std::pair< uvmsc_boost::u16_to_u32_iterator<I>, uvmsc_boost::u16_to_u32_iterator<I> >
   make_utf32_seq(I i, I j, mpl::int_<2> const*)
{
   return std::pair< uvmsc_boost::u16_to_u32_iterator<I>, uvmsc_boost::u16_to_u32_iterator<I> >(uvmsc_boost::u16_to_u32_iterator<I>(i, i, j), uvmsc_boost::u16_to_u32_iterator<I>(j, i, j));
}
template <class I>
inline std::pair< I, I >
...
inline std::pair< uvmsc_boost::u8_to_u32_iterator<const charT*>, uvmsc_boost::u8_to_u32_iterator<const charT*> >
   make_utf32_seq(const charT* p, mpl::int_<1> const*)
{
   std::size_t len = std::strlen((const char*)p);
   return std::pair< uvmsc_boost::u8_to_u32_iterator<const charT*>, uvmsc_boost::u8_to_u32_iterator<const charT*> >(uvmsc_boost::u8_to_u32_iterator<const charT*>(p, p, p+len), uvmsc_boost::u8_to_u32_iterator<const charT*>(p+len, p, p+len));
}
template <class charT>
inline std::pair< uvmsc_boost::u16_to_u32_iterator<const charT*>, uvmsc_boost::u16_to_u32_iterator<const charT*> >
   make_utf32_seq(const charT* p, mpl::int_<2> const*)
{
   std::size_t len = u_strlen((const UChar*)p);
   return std::pair< uvmsc_boost::u16_to_u32_iterator<const charT*>, uvmsc_boost::u16_to_u32_iterator<const charT*> >(uvmsc_boost::u16_to_u32_iterator<const charT*>(p, p, p + len), uvmsc_boost::u16_to_u32_iterator<const charT*>(p+len, p, p + len));
}
template <class charT>
                                 ...
                                 )
{
...
#ifndef UVMSC_BOOST_NO_TEMPLATED_ITERATOR_CONSTRUCTORS
   ...
   else
   {
      ...
      while(i != j)
      {
         ...
         if(f.size())
            out = ::uvmsc_boost::re_detail::regex_format_imp(out, *i, &*f.begin(), &*f.begin() + f.size(), flags, e.get_traits());
         else
            out = ::uvmsc_boost::re_detail::regex_format_imp(out, *i, static_cast<UChar32 const*>(0), static_cast<UChar32 const*>(0), flags, e.get_traits());
         last_m = (*i)[0].second;
...
#include <packages/boost/include/regex/v4/u32regex_iterator.hpp>
#include <packages/boost/include/regex/v4/u32regex_token_iterator.hpp>

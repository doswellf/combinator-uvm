...
#include <packages/boost/include/cregex.hpp>
...
#if defined(UVMSC_BOOST_NO_STDC_NAMESPACE) || defined(__NetBSD__)
namespace std{
#  ifndef UVMSC_BOOST_NO_SWPRINTF
      using ::swprintf;
#  endif
}
#endif
...
typedef uvmsc_boost::basic_regex<wchar_t, c_regex_traits<wchar_t> > wc_regex_type;

UVMSC_BOOST_REGEX_DECL int UVMSC_BOOST_REGEX_CCALL regcompW(regex_tW* expression, const wchar_t* ptr, int f)
{
   ...
   // set default flags:
   uvmsc_boost::uint_fast32_t flags = (f & REG_PERLEX) ? 0 : ((f & REG_EXTENDED) ? wregex::extended : wregex::basic);
   ...
   catch(const uvmsc_boost::regex_error& be)
   {
      result = be.code();
   }
   catch(...)
...
UVMSC_BOOST_REGEX_DECL regsize_t UVMSC_BOOST_REGEX_CCALL regerrorW(int code, const regex_tW* e, wchar_t* buf, regsize_t buf_size)
{
   std::size_t result = 0;
   if(code & REG_ITOA)
   {
      ...
      if((code <= (int)REG_E_UNKNOWN) && (code >= 0))
      {
         ...
         if(buf_size >= result)
#if UVMSC_BOOST_WORKAROUND(UVMSC_BOOST_MSVC, >= 1400) && !defined(_WIN32_WCE) && !defined(UNDER_CE)
...
#if !defined(UVMSC_BOOST_NO_SWPRINTF)
   if(code == REG_ATOI)
   {
      wchar_t localbuf[5];
      if(e == 0)
   ...
   if(code <= (int)REG_E_UNKNOWN)
   {
      ...
      if((e) && (e->re_magic == wmagic_value))
         p = static_cast<wc_regex_type*>(e->guts)->get_traits().error_string(static_cast< ::uvmsc_boost::regex_constants::error_type>(code));
      else
      {
         p = re_detail::get_default_error_string(static_cast< ::uvmsc_boost::regex_constants::error_type>(code));
      }
...
UVMSC_BOOST_REGEX_DECL int UVMSC_BOOST_REGEX_CCALL regexecW(const regex_tW* expression, const wchar_t* buf, regsize_t n, regmatch_t* array, int eflags)
{
...
UVMSC_BOOST_REGEX_DECL void UVMSC_BOOST_REGEX_CCALL regfreeW(regex_tW* expression)
{
   if(expression->re_magic == wmagic_value)
   {

...
#define UVMSC_BOOST_REGEX_V4_BASIC_REGEX_PARSER_HPP
...
void basic_regex_parser<charT, traits>::parse(const charT* p1, const charT* p2, unsigned l_flags)
{
   ...
   if(!result)
   {
      fail(regex_constants::error_paren, ::uvmsc_boost::re_detail::distance(m_base, m_position), "Found a closing ) with no corresponding openening parenthesis.");
      return;
   }
...
void basic_regex_parser<charT, traits>::fail(regex_constants::error_type error_code, std::ptrdiff_t position, std::string message, std::ptrdiff_t start_pos)
{
   ...
   {
      uvmsc_boost::regex_error e(message, error_code, position);
      e.raise();
   }
#else
...
bool basic_regex_parser<charT, traits>::parse_open_paren()
{
   ...
   {
      markid = ++m_mark_count;
#ifndef UVMSC_BOOST_NO_STD_DISTANCE
   ...
   if(m_position == m_end)
   {
      this->fail(regex_constants::error_paren, ::uvmsc_boost::re_detail::distance(m_base, m_end));
      return false;
   }
   UVMSC_BOOST_ASSERT(this->m_traits.syntax_type(*m_position) == regex_constants::syntax_close_mark);
#ifndef UVMSC_BOOST_NO_STD_DISTANCE
...
bool basic_regex_parser<charT, traits>::parse_repeat(std::size_t low, std::size_t high)
{
   ...
   {
      fail(regex_constants::error_badrepeat, ::uvmsc_boost::re_detail::distance(m_base, m_position), "Nothing to repeat.");
      return false;
   }
...
#pragma warning(disable:4127)
   ...
   case regex_constants::syntax_digit:
      ...
      std::ptrdiff_t len = (std::min)(::uvmsc_boost::re_detail::distance(m_position, m_end), static_cast<std::ptrdiff_t>(4));
...
{
   UVMSC_BOOST_ASSERT(m_position != m_end);
   const charT* pc = m_position;
...
bool basic_regex_parser<charT, traits>::unwind_alts(std::ptrdiff_t last_paren_start)
{
   ...
   while(m_alt_jumps.size() && (m_alt_jumps.back() > last_paren_start))
   {
      ...
      this->m_pdata->m_data.align();
      re_jump* jmp = static_cast<re_jump*>(this->getaddress(jump_offset));
      UVMSC_BOOST_ASSERT(jmp->type == syntax_element_jump);

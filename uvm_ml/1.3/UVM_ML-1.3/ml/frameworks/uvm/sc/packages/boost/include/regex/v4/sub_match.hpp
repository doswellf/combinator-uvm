...
#define UVMSC_BOOST_REGEX_V4_SUB_MATCH_HPP
...
struct sub_match : public std::pair<BidiIterator, BidiIterator>
{
...
#if defined(UVMSC_BOOST_NO_STD_ITERATOR_TRAITS) || defined(UVMSC_BOOST_NO_TEMPLATE_PARTIAL_SPECIALIZATION)
   ...
   sub_match(BidiIterator i) : std::pair<BidiIterator, BidiIterator>(i, i), matched(false) {}
               ...
               && !UVMSC_BOOST_WORKAROUND(UVMSC_BOOST_MSVC, < 1310)\
               && !UVMSC_BOOST_WORKAROUND(__BORLANDC__, <= 0x0551)\
               && !UVMSC_BOOST_WORKAROUND(__DECCXX_VER, UVMSC_BOOST_TESTED_AT(60590042))
...
#endif
   difference_type UVMSC_BOOST_REGEX_CALL length()const
   {
      difference_type n = matched ? ::uvmsc_boost::re_detail::distance((BidiIterator)this->first, (BidiIterator)this->second) : 0;
      return n;
   }
   std::basic_string<value_type> str()const
   {
      ...
      if(matched)
      {
         std::size_t len = ::uvmsc_boost::re_detail::distance((BidiIterator)this->first, (BidiIterator)this->second);
         result.reserve(len);
   ...
   bool UVMSC_BOOST_REGEX_CALL operator !=(const sub_match& that)const
   { return compare(that) != 0; }
   bool operator<(const sub_match& that)const
   { return compare(that) < 0; }
   bool operator>(const sub_match& that)const
...
private:
   mutable uvmsc_boost::scoped_ptr<capture_sequence_type> m_captures;
public:
...
#if !(defined(__GNUC__) && defined(UVMSC_BOOST_NO_STD_LOCALE))
template <class RandomAccessIterator>

 ...
 */
  ...
  *   DESCRIPTION: Misc uvmsc_boost::regbase member funnctions.
  */
...
#if defined(UVMSC_BOOST_REGEX_HAS_MS_STACK_GUARD) && defined(_MSC_VER) && (_MSC_VER >= 1300)
#  include <malloc.h>
#endif
...
#if defined(UVMSC_BOOST_REGEX_NON_RECURSIVE) && !defined(UVMSC_BOOST_REGEX_V3)
#if UVMSC_BOOST_REGEX_MAX_CACHE_BLOCKS == 0
#include <new>
#else
#include <packages/boost/include/regex/v4/mem_block_cache.hpp>
#endif
#endif
...
regex_error::regex_error(regex_constants::error_type err) 
   : std::runtime_error(::uvmsc_boost::re_detail::get_default_error_string(err))
   , m_error_code(err)
   , m_position(0) 
{
}
...
void regex_error::raise()const
{
   ...
   ::uvmsc_boost::throw_exception(*this);
#endif
}
...
// error checking API:
//
UVMSC_BOOST_REGEX_DECL void UVMSC_BOOST_REGEX_CALL verify_options(uvmsc_boost::regex::flag_type /*ef*/, match_flag_type mf)
{
...
bool UVMSC_BOOST_REGEX_CALL abstract_protected_call::execute()const
{
   __try{
      return this->call();
...
{
   return ::operator new(UVMSC_BOOST_REGEX_BLOCKSIZE);
}

UVMSC_BOOST_REGEX_DECL void UVMSC_BOOST_REGEX_CALL put_mem_block(void* p)
{
   ::operator delete(p);
}
...
mem_block_cache block_cache = { 0, 0, UVMSC_BOOST_STATIC_MUTEX_INIT, };
#else
...
#if defined(UVMSC_BOOST_RE_USE_VCL) && defined(UVMSC_BOOST_REGEX_DYN_LINK)
